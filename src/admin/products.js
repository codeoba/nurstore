...
async function showProductDetail(ctx, productId) {
  ...
  let text = [
    ...
    `💰 Bei TZS: TZS ${escapeMarkdown(p.priceTzs.toLocaleString('en-US'))}`,
    `💰 Bei USD: $${escapeMarkdown(p.priceUsd.toFixed(2))}`,
    `💸 Punguzo: ${escapeMarkdown(discount)}`,
    `📊 Mauzo: ${p.salesCount}`,
    `📦 Stock: ${p.stock === null ? 'Unlimited' : p.stock}`,
    `👑 VIP Only: ${p.isVipOnly ? 'Ndiyo ✅' : 'Hapana ❌'}`,
    `🔜 Pre\\-Order: ${p.isPreOrder ? 'Ndiyo ✅' : 'Hapana ❌'}`,
    `📊 Hali: ${escapeMarkdown(status)}`,
    `⭐ Featured: ${escapeMarkdown(featured)}`,
